...
for t = 2 : T
    ...
    if UA_flg == 1
        maximize sum( log(THz_HO_Cost) + log(F_QoS_THz) + log(RF_HO_Cost) +  log(F_QoS_RF) )...
    ...
    if UA_flg == 1
        ...
        0.5*log(THz_HO_Cost) + 0.5*log(F_QoS_THz) + 0.5*log(RF_HO_Cost) +  0.5*log(F_QoS_RF) >= log(0.5*1*Rth_vec/THz_BW);
